...
for rounds in [1,4,16]:
    for layers in [1,4]:
        ...
        for hidden_size in hs:
            ...
            train(bitnet_ref(hidden_size=hidden_size, layers=layers),"bitnet_qw_s"+name,hidden_size*layers, train_subset=train_subset, training_mode='qw',lr=1e-3)
            train(bitnet_ref(hidden_size=hidden_size, layers=layers),"bitnet_qw.2_s"+name,hidden_size*layers, train_subset=train_subset, training_mode='qw',lr=2e-4)
