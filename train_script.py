...
for rounds in [1,4,16]:
    for layers in [1,4]:
        ...
        for hidden_size in hs:
            ...
            train(bitnet_ref(hidden_size=hidden_size, layers=layers),"bitnet_qwl.02_s"+name,hidden_size*layers, train_subset=train_subset, training_mode='qw',lr=2e-5)
            train(bitnet_ref(hidden_size=hidden_size, layers=layers),"bitnet_sgd.02_s"+name,hidden_size*layers, train_subset=train_subset, training_mode='sgd', lr=2e-5)
