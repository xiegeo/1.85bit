...
for rounds in [1,4,16]:
    for layers in [1]:
        ...
        for lr in lrs:
            ...
            name = f'lr_{lr}_l_{layers}_hs_{hidden_size}'
...
for rounds in [1,4,16,64]:
    for layers in [1]:
        ...
        for hidden_size in hs:
            ...
            train(bitnet_ref(hidden_size=hidden_size, layers=layers),"bitnet_qwl_s"+name,hidden_size*layers, train_subset=train_subset, optimizer_function=SGDFun(), QW=True)
            train(bitnet_ref(hidden_size=hidden_size, layers=layers),"bitnet_sgd_s"+name,hidden_size*layers, train_subset=train_subset, optimizer_function=SGDFun(), QW=False)
            ...
            train(bitnet_ref(hidden_size=hidden_size, layers=layers),"bitnet_s"+name,hidden_size*layers, train_subset=train_subset, QW=False)
            BitLinear.default_stochastic_rounding = False
            # train(llama_ref(hidden_size=hidden_size, layers=layers),"llama"+name,hidden_size*layers, train_subset=train_subset)
            train(bitnet_ref(hidden_size=hidden_size, layers=layers),"bitnet_qw"+name,hidden_size*layers, train_subset=train_subset, QW=True)
            train(bitnet_ref(hidden_size=hidden_size, layers=layers),"bitnet"+name,hidden_size*layers, train_subset=train_subset, QW=False)
